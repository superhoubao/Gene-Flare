...
test('top app bar uses the new command-bar visual language', () => {
  ...
  assert.doesNotMatch(language, /指令层|Command Layer|基因之火/);
  assert.match(language, /defaultTitle: 'MIO'/);
});
...
test('project branding is unified under MIO', () => {
  ...
  files.forEach((file) => {
    const source = read(file);
    assert.doesNotMatch(source, /MIO Health|mio_health|基因之火/, `${file} should not contain legacy brand text`);
  });

  assert.match(read('package.json'), /"name": "mio_app"/);
  assert.match(read('package-lock.json'), /"name": "mio_app"/);
  assert.match(read('index.html'), /<title>MIO<\/title>/);
  assert.match(read('PROJECT_PAGES.md'), /# MIO 项目页面说明文档/);
});
...
test('language context supports interpolation, fallback, and persisted language switching', () => {
  ...
  assert.match(language, /mio_lang/);
