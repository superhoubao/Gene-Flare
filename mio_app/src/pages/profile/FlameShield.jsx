...
export default function FlameShield() {
  ...
  const luxuryServices = [
    ...
    },
    {
      ...
      unlocked: demoState.isGenesisCompleted, // 创世通关后解锁
    },
    {
  ...
  // 动态切换看板数据
  const stats = [
    { label: t('shieldPage.currentTier'), value: demoState.isGenesisCompleted ? 'Gold 3' : 'Bronze 1' },
    { label: t('shieldPage.mutualPool'), value: demoState.isGenesisCompleted ? '450 GEF' : '20 GEF' },
    { label: t('shieldPage.benefitScore'), value: demoState.isGenesisCompleted ? '95 / 100' : '35 / 100' },
    { label: t('shieldPage.unlocked'), value: demoState.isGenesisCompleted ? t('shieldPage.perks') : '1 / 3' },
  ];

  return (
    <div className="min-h-screen -mx-6 px-6 pt-2">
      <main className="space-y-6 pb-8">
        {/* The Metal Black Card Dashboard — 根据状态切换尊贵度 */}
        <section className={`group relative overflow-hidden rounded-[38px] p-7 text-white transition-all duration-700 lg:p-8 border
          ${demoState.isGenesisCompleted 
            ? 'bg-gradient-to-br from-[#0a0a0a] via-[#1a1610] to-[#0a0a0a] border-[#4f3b23] shadow-[0_32px_80px_-12px_rgba(217,119,6,0.25)]' 
            : 'bg-gradient-to-br from-[#0a0a0a] via-[#12141a] to-[#0a0a0a] border-[#22242b] shadow-[0_24px_64px_-12px_rgba(0,0,0,0.4)] opacity-95'
          }`}
        >
          {/* Holographic Shimmer Effect */}
          <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/[0.02] to-transparent -translate-x-full group-hover:translate-x-full duration-[2000ms] transition-transform ease-in-out" />
          
          {/* 状态对应光晕背景 */}
          {demoState.isGenesisCompleted ? (
            <div className="absolute -right-10 -top-10 h-48 w-48 rounded-full bg-amber-500/15 blur-[60px] animate-pulse" />
          ) : (
            <div className="absolute -right-10 -top-10 h-48 w-48 rounded-full bg-slate-500/5 blur-[60px]" />
          )}
          ...
          <div className="relative z-10 flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-[430px]">
              <div className="flex items-center gap-3">
                ...
                <span className={`rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[0.2em] shadow-md
                  ${demoState.isGenesisCompleted 
                    ? 'bg-gradient-to-r from-amber-400 to-amber-600 text-[#1a1610] shadow-[0_0_12px_rgba(251,191,36,0.5)]' 
                    : 'bg-slate-800 border border-slate-700 text-slate-400'
                  }`}
                >
                  {demoState.isGenesisCompleted ? t('shieldPage.vip') : 'INIT'}
                </span>
              </div>
              <h1 className="mt-4 text-[3.1rem] font-black tracking-[-0.05em] leading-[0.92] md:text-[4.2rem]">
                {demoState.isGenesisCompleted ? t('shieldPage.heroTitle') : (language === 'zh' ? '烈焰之盾' : 'Flame Shield')}
              </h1>
              <p className="mt-4 text-sm leading-relaxed text-amber-50/70 font-medium">
                {demoState.isGenesisCompleted 
                  ? t('shieldPage.heroDesc') 
                  : (language === 'zh' 
                      ? '在创世阶段中，通过日常行为打卡和零知识证明同步，逐步炼制并苏醒您的专属烈焰之盾。' 
                      : 'Build your personal cryptographic protection shield dynamically through daily biometrics commitment.'
                    )
                }
              </p>
            </div>
            ...
            <div className="grid w-full grid-cols-2 gap-3 lg:w-auto lg:translate-y-8">
              {stats.map((item, index) => (
                <div key={item.label} className={`signal-chip rounded-[24px] bg-white/5 border border-white/10 p-3 sm:p-4 backdrop-blur-xl transition-all duration-500 ${index % 2 === 0 ? 'lg:-translate-y-4' : ''}`}>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[34px] border border-slate-100 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-amber-50 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-amber-700 border border-amber-200/50">
                {demoState.isGenesisCompleted ? t('shieldPage.diamondProgress') : (language === 'zh' ? '创世锁死中' : 'GENESIS PENDING')}
              </span>
            </div>
            <div className="mt-6 h-2.5 overflow-hidden rounded-full bg-slate-100 shadow-inner relative">
              <div 
                className={`absolute inset-y-0 left-0 rounded-full transition-all duration-1000 shadow-lg
                  ${demoState.isGenesisCompleted 
                    ? 'bg-gradient-to-r from-amber-500 via-amber-400 to-yellow-300 w-[72%] shadow-[0_0_12px_rgba(251,191,36,0.8)]' 
                    : 'bg-gradient-to-r from-slate-400 to-slate-300 w-[12%] shadow-none'
                  }`}
              >
          ...
          <div className="relative overflow-hidden rounded-[34px] border border-[#2a241b] bg-[#0c0a08] p-6 text-amber-100 shadow-[0_16px_40px_rgba(0,0,0,0.4)]">
            <div className="absolute -left-10 -top-10 h-32 w-32 rounded-full bg-amber-600/10 blur-3xl text-amber-500"></div>
